...
async function start(): Promise<void> {
  ...
  for (let emoji of emojiPack.emojis) {
    ...
    const imagePath = `${__dirname}/emoji.src`;
...
async function upload(
  ...
  const addEmojiButtonElement = await page.waitForSelector(
    ".p-customize_emoji_wrapper .c-button--primary",
    { visible: true }
  );
  ...
  const saveEmojiButtonElement = await page.$(
    ".c-dialog__footer .c-button--primary"
  );
  await saveEmojiButtonElement.click();

  await page.waitForSelector(".c-dialog__footer .c-button--primary", {
    hidden: true
  });
}
...
async function setInputElementValue(
  ...
  return page.$$eval(
    querySelector,
    (element: HTMLInputElement[], v) => (element[0].value = v),
    value
  );
}
